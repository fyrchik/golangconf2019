...
func (c *SL2) mul(a, b *SL2, x *[4]GF127) *SL2 {
	...
	Mul(&a[0][1], &b[1][0], &c[0][0])
	Add(&c[0][0], &x[0], &c[0][0])
	Mul(&a[0][1], &b[1][1], &c[0][1])
	Add(&c[0][1], &x[1], &c[0][1])
	Mul(&a[1][1], &b[1][0], &c[1][0])
	Add(&c[1][0], &x[2], &c[1][0])
	Mul(&a[1][1], &b[1][1], &c[1][1])
	Add(&c[1][1], &x[3], &c[1][1])
...
func (c *SL2) MulB() *SL2 {
	...
	Mul1(&c[0][0], &a)
	Mul10(&c[0][0], &c[0][0])
	Add(&c[0][1], &c[0][0], &c[0][0])
	Add(&c[0][0], &a, &c[0][1])

	Mul1(&c[1][0], &a)
	Mul10(&c[1][0], &c[1][0])
	Add(&c[1][1], &c[1][0], &c[1][0])
	Add(&c[1][0], &a, &c[1][1])
